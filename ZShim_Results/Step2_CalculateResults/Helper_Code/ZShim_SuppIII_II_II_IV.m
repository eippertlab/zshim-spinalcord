function ZShim_SuppIII_II_II_IV(figurepath,rawdatapath,processdatapath,recalculateResults,fslDir)
% Find subjects for which the FM-based approach led to the most extreme
% differences compared to manual z-shim (>= 3 steps).
% Plot local field offset and absolute difference between automated and
% manual z-shim values (Supplementary figure 4).
...
% figurepath:        fullpath, string, output location (to save the figure)
...
if recalculateResults
    ...
    load(fullfile(processdatapath, 'extracted_signal', 'signal_templatespace', 'GroupWhole', ...
        'ReconstructedSignal', 'FM_inhomogeneity', 'results.mat'))
