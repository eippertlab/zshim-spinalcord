function handles = plotSpread(faceColor,varargin)
...
for iData = 1:nData
    for iCategory = 1:nCategories
        ...
        if any(currentIdx)
            switch opt.xyOri
                case 'normal'
                    ph(iData,iCategory) = scatter(data(currentIdx,1),data(currentIdx,2),25,'filled', ...
       'MarkerFaceAlpha',3/8,'MarkerFaceColor', faceColor)         
                    
                    
                    
                    %plot(ah,data(currentIdx,1),...
                     
                   %data(currentIdx,2), '.', ...
                    %    'Color',[1, 0, 0],...
                     %   'lineStyle','none',...
                      %  'DisplayName',plotLabels{iData,iCategory});
                                        %'marker',plotMarkers{iData,iCategory},'markersize',10,...
