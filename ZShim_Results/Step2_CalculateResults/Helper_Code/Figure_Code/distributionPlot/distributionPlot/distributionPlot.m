...
for iData = goodData'
    ...
    switch opt.xyOri
        case 'normal'
            hh(iData)= patch('Vertices',vertices,'Faces',faces,'Parent',ah,colorOpt{:},'EdgeColor','none', 'FaceAlpha',.3);
        case 'flipped'
            hh(iData)= patch('Vertices',vertices(:,[2,1]),'Faces',faces,'Parent',ah,colorOpt{:},'EdgeColor','none', 'FaceAlpha',.3);
    end
...
if opt.addSpread
    if isHistogram
        ...
        % add spread
        try
            sh = plotSpread(ah,data,'xValues',opt.xValues,'xyOri',opt.xyOri,  'spreadWidth', 0.2);
            set(sh{1}(ishandle(sh{1})),'color',[0.8,0.8,0.8]);
        catch me
    ...
    switch opt.xyOri
        case 'normal'
            if any(opt.showMM==[1,2])
                mh = plot(ah,opt.xValues+xOffset,m,'o','Color',[0.5,0.5, 0.5],'MarkerSize',8, 'MarkerFaceColor', [0.5 0.5 0.5]);
            end
